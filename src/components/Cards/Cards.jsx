...
function Cards({data}) {
  return (
    <div key={data.userId} className='w-[294px] h-[406px] flex flex-col gap-2.5'>
      <div style={{ backgroundImage: `url(${data.blogImage})`}} className={`relative rounded-[5px] min-h-[198px] w-full shadow-[0px_1px_7px_0px_#213555] 
      ...
      <div className='w-full h-full pb-2.5 px-2.5 flex gap-2.5 flex-col'>
        ...
        <div className='w-full h-full flex flex-col gap-[5px]'>
          ...
          <div className='w-full h-full flex items-center justify-center gap-2.5'>
            <p className='ttext-center italic text-[11px] leading-[10px]'>
              {data.body.substring(0, 200)+" ..."}
            </p>
          </div>
        </div>
        <Link href={`view/${data.userId}`} style={{width: 68, height: 19, paddingBottom: 18}} className='pt-2.5 px-[5px] border-b-[1px] border-b-[#000000]
